...
class TestMB01PDScale:
    ...
    def test_scale_full_matrix_small_norm(self):
        ...
        m, n = 3, 3
        a = np.array(
            [[1e-300, 2e-300, 3e-300], [4e-300, 5e-300, 6e-300], [7e-300, 8e-300, 9e-300]],
            dtype=np.float64,
            order="F",
        )
        anrm = 1e-300
        ...
        a_out, info_out = mb01pd(1, 0, m, n, 0, 0, anrm, 0, nrows, a)
    ...
    def test_scale_full_matrix_large_norm(self):
        ...
        m, n = 2, 2
        a = np.array([[1e300, 2e300], [3e300, 4e300]], dtype=np.float64, order="F")
        anrm = 1e300
    ...
    def test_scale_lower_triangular(self):
        ...
        m, n = 3, 3
        a = np.array(
            [[1e-300, 0.0, 0.0], [2e-300, 3e-300, 0.0], [4e-300, 5e-300, 6e-300]],
            dtype=np.float64,
            order="F",
        )
        anrm = 1e-300
        ...
        a_out, info_out = mb01pd(1, 1, m, n, 0, 0, anrm, 0, nrows, a)
    ...
    def test_scale_upper_triangular(self):
        ...
        m, n = 3, 3
        a = np.array(
            [[1e-300, 2e-300, 3e-300], [0.0, 4e-300, 5e-300], [0.0, 0.0, 6e-300]],
            dtype=np.float64,
            order="F",
        )
        anrm = 1e-300
        ...
        a_out, info_out = mb01pd(1, 2, m, n, 0, 0, anrm, 0, nrows, a)
...
class TestMB01PDUnscale:
    ...
    def test_unscale_after_scale(self):
        ...
        m, n = 2, 2
        a_orig = np.array([[1e-300, 2e-300], [3e-300, 4e-300]], dtype=np.float64, order="F")
        anrm = 1e-300
...
class TestMB01PDEdgeCases:
    ...
    def test_rectangular_matrix(self):
        ...
        m, n = 3, 2
        a = np.array(
            [[1e-300, 2e-300], [3e-300, 4e-300], [5e-300, 6e-300]], dtype=np.float64, order="F"
        )
        anrm = 1e-300
